	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if (action.equals("Register")) {
				...
				String gpa = request.getParameter("gpa");
				String year = request.getParameter("year");
				jdbc.modifyUser(userid, password, firstname, lastname, prefname, Boolean.parseBoolean(shortterm), Boolean.parseBoolean(longterm), Double.parseDouble(gpa), Integer.parseInt(year));
			} else if (action.equals("Info")) {
				String userid = request.getParameter("uid");
				Account results = jdbc.userInfo(userid);
				Gson gson = new Gson();
